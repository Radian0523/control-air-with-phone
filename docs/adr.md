...
## ADR-020: 障害系の個別試験を省略し、通常運用での観察に置き換える

状態: Accepted（2026-09-17）

### 文脈

段階5として、誤トークン、誤ホスト、未信頼証明書、AP断からの復帰、電源OFF後の503、24時間の接続失敗放置、TLS所要時間とRAMの実測を計画していた。一方、段階3・4で「Wi-Fi失敗時に再起動せず約45秒周期で再試行する」「通し操作でエアコンが動く」ことは既に実機で確認できている。個人用1台の機器であり、所有者は動作確認にかける時間より早く使い始めることを優先した。

### 決定

段階5の個別試験を実施しない。代わりに、正常接続の状態でふだん使いを続け、アプリから操作できることをもって運用可否を判断する。不具合が出た時点で、該当する試験を docs/stage5-procedure.md から選んで実施する。

### 結果

- 設計 §10 の「実装前・設置前に確認すること」のうち、障害系の項目は完了条件から外す
- 未信頼証明書で接続が失敗することは未確認である。CA bundle は非 null と実サイズを渡す実装で、null 時は起動を止めるコードになっているが、検証が実際に働いているかの実機確認はしていない
- ルーター再起動からの自動復帰は未確認である。復帰しない場合は ESP32 の電源を入れ直す運用になる
- 長時間運用での heap 推移は未確認である。再起動ループや無応答が起きた場合に、試験用ビルド（AIRCON_DEBUG_HEAP）で調べる
- 試験手順書と試験用ビルドは残し、必要になった時に使う
