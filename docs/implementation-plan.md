...
| 2a | 実機リモコン採取 | 完了（2026-09-16）。規則確定、fan 列挙を auto/1/2/3 へ変更（ADR-019） |
| 2b | 回路組立と電気試験 | 完了（2026-09-17）。ADC 測定で全項目が期待範囲内。docs/test-log.md |
| 2c | シリアル試験スケッチ | 完了（2026-09-17）。5ベクトル全件・設置位置から反応。docs/test-log.md |
| 3 | 機器接続 | 完了（2026-09-17）。誤トークン/誤ホスト/TLS 時間/RAM の実測は段階5へ移管 |
| 4 | 即時操作 | 完了（2026-09-17）。curl → 202 → ir_sent → エアコン動作。docs/test-log.md |
| 5 | 障害系 | 省略（ADR-020、2026-09-17）。通常運用での観察に置き換え。手順書と試験用ビルドは保持 |
| 6 | iOS | 進行中。設定・操作画面と8ユニットテスト作成、シミュレータで表示確認（2026-09-17）。実機での操作確認が残り |
| 7 | 予約 | 実装・統合テスト・デプロイ済み（2026-09-18）。iOS 予約画面あり。実機での発火確認が残り |
| 8 | 設置 | 未着手 |
