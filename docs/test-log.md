...
## 段階6・7. iOS アプリと予約（2026-09-18）

| 項目 | 結果 |
|---|---|
| iOS ユニットテスト（JSON 形式、列挙値、executeAt 形式、文言方針） | 合格（12件） |
| Cloudflare 統合テスト（/command、/schedule、Alarm の二重送信なし） | 合格（49件） |
| iPhone 実機からアプリで操作し、エアコンが動く | 合格 |
| アプリから予約し、指定時刻にエアコンが動き、予約が消える | 合格 |
| ESP32 の給電を切って戻すと、手を触れずに再接続する | 合格（所有者確認。段階5 T6 の一部に相当） |

### 判定

段階6・7のゲートを満たす。残りは段階8（設置）のみ。

## 段階8. 設置（2026-09-18）

| 項目 | 結果 |
|---|---|
| 配線を固定し、USB ポートへ手が届く位置に恒久設置 | 合格 |
| 設置位置からアプリで操作し、エアコンが動く | 合格 |
| 設置位置から予約が発火する | 合格 |
| 電源断からの自動再接続 | 合格（段階7で確認） |

### 判定

design.md §10 の完了条件を満たす（所有者確認）。プロジェクトの実装フェーズを完了とする。
ADR-020 により省略した障害系試験は、通常運用で問題が出た時に docs/stage5-procedure.md から選んで実施する。
