...
- 予約はGET、PUT、DELETEの3操作だけとし、操作画面の中に置く（別画面は設定だけ）
...
障害系の項目（AP断、誤証明書、24時間失敗、TLS時間、RAM）はADR-020により完了条件から外し、通常運用での観察に置き換える。試験手順は[段階5手順書](./stage5-procedure.md)に残す。
