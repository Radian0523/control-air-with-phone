...
実機リモコンに合わせ、byte 9のbit 7（IRremoteESP8266の`FanAuto`）は常に0とする。
...
| ON / cool / 26℃ / auto / auto | 規則から導出 | `23CB26010020180A364000000000001000DD` |
| OFF / cool / 26℃ / auto / auto | 規則から導出 | `23CB26010000180A364000000000001000BD` |
| ON / heat / 20℃ / 4 / low | 規則から導出 | `23CB260100200804306400000000001000E5` |

4件が一致するまでネットワーク処理へ組み込まない。導出ベクトルはencoderが規則どおりであることを確認するもので、実機互換性の証明にはしない。

ネットワーク実装前に、実機リモコンから少なくとも次を採取する。

- 静音のraw byte 9にあるfan値を採取し、生成規則の5と一致するか照合する
- 暖房20℃、風量4、風向low
- 電源ONとOFFを含む複数設定のbyte 10〜16

採取結果と異なる場合は、テストを無理に通さず生成規則を実機値へ合わせる。特にbyte 10〜16へ時計やタイマー由来の値が入る場合は、固定値としてよい範囲を再判断する。
...
| 2. 赤外線 | 回路、実機リモコン採取、USBシリアルから36文字を受ける試験コード | 静音・暖房・固定部を採取確認し、実機が安定して反応して電気試験合格 |
